...
struct DenseFsa {
  ...
  Arc arc(int32_t arc_index) const {
    int32_t original_arc_index = arc_index;
   ...
   */
  DenseFsa(int32_t T, int32_t num_symbols, int32_t arc_offset, const float *data)
...
struct DenseFsaVec {
  ...
  DenseFsa operator[](int32_t seg_id) const {
    ...
    return DenseFsa(T, meta->num_symbols, arc_offset, this->data);
  }
};
